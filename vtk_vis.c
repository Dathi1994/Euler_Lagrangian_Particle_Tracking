...
PetscErrorCode Step_TimeIteration(UserCtx *user, PetscInt ti, PetscInt tis, PetscInt tsteps)
{
	...
	PetscReal v_scale = 5.5;
	PetscReal density_particle = 1602;
	PetscReal diameter_particle = 30e-6;
	PetscReal density_fluid = 1.168;
	//PetscReal mass_particle = 6.7104e-9;
	PetscReal mass_particle;
	PetscReal T_physical = 3.694e-4;
...
*/			uploc[particle] = user[bi].uploc[particle];
			...
			*/
			...
			if (!interpolation) {
				...
				Fb_x = 0;
				Fb_y = 0;
				Fb_z = 0;//((0.166)*3.14*pow((diameter_particle),3)*(density_particle - density_fluid)*9.81);//*(pow(length_scale,2)*density_fluid*pow(v_scale,2));
				//Fb_z = mass_particle*9.81*(1-(density_fluid/density_particle));
...
//----------------------------------------- shear induced lift force starts -------------------------------------------------------------------------------------------------------//
...
*/
				...
				Compute_du_dxyz (csi0, csi1, csi2, eta0, eta1, eta2, zet0, zet1, zet2, ajc,
			        ...
			        PetscReal w_1, w_2, w_3, w_f;
				PetscReal u_cross_w_1 ,u_cross_w_2, u_cross_w_3;	
				//PetscReal F_SL_x, F_SL_y, F_SL_z;
				PetscReal C_sl, Re_sh, beta_sh;
				PetscReal F_SL_x, F_SL_y, F_SL_z;

                                w_1 =  dw_dy - dv_dz;
			        w_2 =  du_dz - dw_dx;
				w_3 =  dv_dx - du_dy;
				w_f = sqrt (pow(w_1,2) + pow(w_2,2) + pow(w_3,3));//Angular velocity of fluid
				Re_sh = (density_fluid*pow(diameter_particle,2)*w_f)/(viscosity_fluid);//particle reynolds number of the shear flow
				
				u_cross_w_1 = (fluid_velocity_y - vploc[particle])*(du_dy) + (fluid_velocity_z - wploc[particle])*(du_dz) - (fluid_velocity_y - vploc[particle])*(dv_dx) - (fluid_velocity_z - wploc[particle])*(dw_dx); // (u x w)_1 

				u_cross_w_2 = (fluid_velocity_x - uploc[particle])*(dv_dx) + (fluid_velocity_z - wploc[particle])*(dv_dz) - (fluid_velocity_x - uploc[particle])*(du_dy) - (fluid_velocity_z - wploc[particle])*(dw_dy); //(u x w)_2

				u_cross_w_3 = (fluid_velocity_x - uploc[particle])*(dw_dx) + (fluid_velocity_y - vploc[particle])*(dw_dy) - (fluid_velocity_x - uploc[particle])*(du_dz) - (fluid_velocity_y - vploc[particle])*(dv_dz); //(u x w)_3
																										       beta_sh = 0.5*(Re_sh/Re_s);
				
		     	        if (Re_s <= 40){

					C_sl = (1 - 0.331*sqrt(beta_sh))*exp(-0.1*(Re_s)) + 0.3314*pow(beta_sh,0.5);
				}
				else if (Re_s > 40){

					C_sl = 0.0524*sqrt(beta_sh*Re_s);
				}																				       
				F_SL_x = 1.615*diameter_particle*viscosity_fluid*pow(Re_sh,0.5)*C_sl*u_cross_w_1*(pow(v_scale,2)/length_scale);//shear-induced lift force
 				F_SL_y = 1.615*diameter_particle*viscosity_fluid*pow(Re_sh,0.5)*C_sl*u_cross_w_2*(pow(v_scale,2)/length_scale);
 				F_SL_z = 1.615*diameter_particle*viscosity_fluid*pow(Re_sh,0.5)*C_sl*u_cross_w_3*(pow(v_scale,2)/length_scale);

//---------------------------------------------------------------------- shear flow ends --------------------------------------------------------------------------------------------//

//------------------------------------------------------------------ angular momentum starts ---------------------------------------------------------------------------------------//
				PetscReal I_p, Omega_x, Omega_y, Omega_z, Omega, w_px[particle], w_py[particle], w_pz[particle], T_x, T_y, T_z, C_r;
				PetscReal Re_r, w_pxite[particle], w_pyite[particle], w_pzite[particle];
				
				I_p = mass_particle*pow(diameter_particle,2);// Momentum of Inertia of sphere

				//w_px[particle] =0;
				//w_py[particle] =0;
				//w_pz[particle] =0;

				Omega_x = 0.5*w_1 - w_px[particle];// 0.5*del x u_f - w_p
				Omega_y = 0.5*w_2 - w_py[particle];
				Omega_z = 0.5*w_2 - w_pz[particle];

				Omega = sqrt(pow(Omega_x,2) + pow(Omega_y,2) + pow(Omega_z,2));

				Re_r = (density_fluid*pow(diameter_particle,2)*Omega)/viscosity_fluid;
				...
				if (Re_s <= 32){
					...
					C_r = (12.9/sqrt(Re_r)) + (128.4/Re_r);
				}

				T_x = C_r*(density_fluid/2)*pow((diameter_particle/2),5)*Omega*Omega_x;// Torque acting on a rotating particle
				T_y = C_r*(density_fluid/2)*pow((diameter_particle/2),5)*Omega*Omega_y;
				T_z = C_r*(density_fluid/2)*pow((diameter_particle/2),5)*Omega*Omega_z;

				w_pxite[particle] = w_px[particle] + (T_x/I_p)*nondtsteps;
				w_pyite[particle] = w_py[particle] + (T_y/I_p)*nondtsteps;
				w_pzite[particle] = w_pz[particle] + (T_z/I_p)*nondtsteps;
...
//------------------------------------------------------------------- angular momentum----------------------------------------------------------------------------------------------//				
				...
				acceleration_x_temp = ((Fpx[particle] + Fb_x +  FD_x + FA_x + F_SL_x)/mass_particle)*(length_scale/pow(v_scale,2));

			        acceleration_y_temp = ((Fpy[particle] + Fb_y +  FD_y + FA_y + F_SL_y)/mass_particle)*(length_scale/pow(v_scale,2));

				acceleration_z_temp = ((Fpz[particle] + Fb_z +  FD_z + FA_z + F_SL_z)/mass_particle)*(length_scale/pow(v_scale,2));
			...
			*/
			...
			/*	
				      ...
				      */
				      ...
				      vt_x = (0.3*uploc_temp +  vn_new_x);
				      vt_y = (0.3*vploc_temp +  vn_new_y);
				      vt_z = (0.3*wploc_temp +  vn_new_z);
				      ...
				      uploc[particle] = (vt_x + 0.5*vn_new_x);
				      vploc[particle] = (vt_y + 0.5*vn_new_y);
				      wploc[particle] = (vt_z + 0.5*vn_new_z);
				      
				      w_px[particle] = uploc[particle]/diameter_particle;
				      w_py[particle] = vploc[particle]/diameter_particle; 
				      w_pz[particle] = wploc[particle]/diameter_particle;
			...
			// Check if particle has reached the maximum k value

			PetscInt max_k = 135;
    			if (kp[particle] >= max_k) {
        		// Mark the particle as inactive by setting its position to an invalid state
        			xpite[particle] = -1;
        			ypite[particle] = -1;
        			zpite[particle] = -1;
			}

			// Reinitialize particles that were marked as inactive
			for (int particle = 0; particle < NUM_OF_PARTICLE; particle++) {
			
				PetscInt    min=0, max=3;
				PetscInt    kpini=20, jpini, ipini;
				srand(particle+kpini);
				...
				if (xpite[particle] == -1 && ypite[particle] == -1 && zpite[particle] == -1) {
					// Reinitialize the particle at the starting k value
                                	//kp[particle] = zpini[particle];//starting_k;
                                	xpite[particle] = xpini[particle];//initial_x_position; // Define these initial values appropriately
                                	ypite[particle] = ypini[particle];//initial_y_position;
                                	zpite[particle] = zpini[particle];//initial_z_position;
                        	}
                	}
	...
	//Release the vector from the memory
	//	DAVecRestoreArray(user[bi].da, user[bi].Aj, &aj);	
	//	DAVecRestoreArray(user[bi].fda,user[bi].Csi, &csi);
	//	DAVecRestoreArray(user[bi].fda, user[bi].Eta, &eta);
	//	DAVecRestoreArray(user[bi].fda, user[bi].Zet, &zet);
